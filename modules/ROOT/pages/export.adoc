...
The {pluginname} plugin adds the ability to export content from the editor to a user's local machine in various formats. For a list of available exporters and information on what they support, see the xref:#_exporters[Exporters section].
...
To add the {pluginname} plugin to the editor, add `+pass:a[{plugincode}]+` to the `+plugins+` option in the editor configuration.
...
* xref:#_client_side_pdf_clientpdf[Client-side PDF (`+clientpdf+`)]
...
* Remote images require an image proxy to render due to browser limitations. For information on proxying remote images, see the xref:#_export_image_proxy[export_image_proxy] option.
