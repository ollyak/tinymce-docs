...
* xref:setting_up_jwt_authentication_for_real_time_collaboration[Setting up JWT authentication for Real-Time Collaboration]
* xref:add_a_public_key_to_the_tiny_cloud_api_key[Add a public key to the {cloudname} API key]
...
[[setting_up_jwt_authentication_for_real_time_collaboration]]
...
* The JWTs are generated (signed) using the _private_ key that pairs with the _public_ key generated at (or provided to) {jwturl}[{accountpage} - JWT Keys].
...
[[required_jwt_claims_for_real_time_collaboration]]
