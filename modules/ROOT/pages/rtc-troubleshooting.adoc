...
As noted in the xref:rtc-options-required.adoc#_rtc_document_id[Document ID configuration option], if a document ID is not known to the server, new initial data will be uploaded. If two users manage to do this at the same time, one will be forced to wait for the other to establish the session. This has implications for configurations using a dynamic xref:rtc-options-required.adoc#_rtc_encryption_provider[`+rtc_encryption_provider+`].
...
For a list of supported algorithms, see: xref:rtc-jwt-authentication.adoc#_generate_a_key_pair_locally[RTC JWT Authentication - Supported algorithms].
...
* The payload part does not contain all xref:rtc-jwt-authentication.adoc#_required_jwt_claims_for_real_time_collaboration[required claims]
