...
Once you have the link:introduction-to-premium-selfhosted-services.html[server-side component] installed, additional configuration to your `+application.conf+` file is required. (Don't forget to restart the Java application server after updating the configuration.)
...
A summary card is an embeddable snippet of code which is generated based on what the Enhanced Media Embed service can work out about the content at the URL. See the integration docs for link:mediaembed-server-integration.html[Enhanced Media Embed Server] for further details.
