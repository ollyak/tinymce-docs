...
____
If you haven't tried any of the xref:tinydrive-getting-started.adoc#_starter_projects[Starter projects] yet, we urge you to try them before trying to implement your solution. The source is also available on Github to study.
____

== Overview

* xref:#_setting_up_jwt_authentication_for_tiny_drive[Setting up JWT authentication for Tiny Drive]
* xref:#_add_a_public_key_to_the_tiny_cloud_api_key[Add a public key to the Tiny Cloud API key]
* xref:#_set_up_a_json_web_token_jwt_endpoint[Set up a JSON Web Token (JWT) endpoint]
* xref:#_jwt_endpoint_examples_[JWT endpoint examples]
...
* xref:#_php_token_provider_endpoint_example[PHP token provider endpoint example]
* xref:#_node_js_token_provider_endpoint_example[Node.js token provider endpoint example]
