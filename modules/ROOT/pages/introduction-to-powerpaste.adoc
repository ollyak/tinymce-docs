...
If you configure *PowerPaste* to allow local images (see the xref:powerpaste_options.adoc#_powerpaste_allow_local_images[`+powerpaste_allow_local_images+`] setting below), then images copied from Microsoft Word and other sources will appear in {productname} as Base64 encoded images. You can have {productname} automatically upload Base64 encoded images for reverting back to a standard image as described in the xref:upload-images.adoc[image upload documentation].
