...
The service URL is already configured with link:editor-and-features.html[{cloudname}]. Simply specify the `+media+` and `+mediaembed+` plugins, and optionally a `+mediaembed_max_width+`
...
A link:{pricingpage}/[{enterpriseversion} subscription] includes the ability to download and install the *Media Embed* plugin and a *WAR* file to access the service backend. Please follow these link:introduction-to-premium-selfhosted-services.html#step6setupeditorclientinstancestousetheserver-sidefunctionality[instructions] to configure the *WAR* file.
