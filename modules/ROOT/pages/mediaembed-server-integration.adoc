...
See the docs on xref:mediaembed-server-config.adoc#_configure_a_custom_endpoint[configuring a custom endpoint] for details on getting the Enhanced Media Embed server to utilize your custom API.

=== OEmbed endpoint

This is a popular choice and many CMSs have existing plugins that support oEmbed. While you can create custom HTML embeds this way, they cannot contain scripts. If the HTML contains a script, then the Enhanced Media Embed server will embed a xref:mediaembed-server-config.adoc#_summary_cards[summary card].
...
The other option for developing a custom API endpoint is to return JSON in the xref:#_tinymce_enhanced_media_embed_format[{productname} Enhanced Media Embed format].
...
* HTTP 200 (OK): xref:#_ephoxembedobj[`+EphoxEmbedObj+`]
* HTTP 400 (User Error): xref:#_errorobj[`+ErrorObj+`]
* HTTP 503 (Upstream Error): xref:#_errorobj[`+ErrorObj+`]
* HTTP 500 (Unexpected Error): xref:#_errorobj[`+ErrorObj+`]
...
* `+links+` (required)
** xref:#_linksobj[LinksObj]
* `+rel+` (optional)
** xref:#_relobj[RelObj]
* `+media+` (optional)
** xref:#_mediaobj[MediaObj]
* `+html+` (optional)
...
* `+primary+` (required)
** Array of xref:#_primaryrel[PrimaryRel]s
* `+technical+` (required)
** Array of xref:#_technicalrel[TechnicalRel]s
* `+source+` (required)
** Array of xref:#_sourcerel[SourceRel]s
...
* `+players+` (required)
** Array of xref:#_linkobj[LinkObj]s
* `+thumbnails+` (required)
** Array of xref:#_linkobj[LinkObj]s
* `+apps+` (required)
** Array of xref:#_linkobj[LinkObj]s
...
** Integer
* `+width+` (required)
** xref:#_dimensionboundobj[DimensionBoundObj]
* `+height+` (required)
** xref:#_dimensionboundobj[DimensionBoundObj]
