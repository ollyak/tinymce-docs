...
See the docs on link:mediaembed-server-config.html#_configure_a_custom_endpoint[configuring a custom endpoint] for details on getting the Enhanced Media Embed server to utilize your custom API.

=== OEmbed endpoint

This is a popular choice and many CMSs have existing plugins that support oEmbed. While you can create custom HTML embeds this way, they cannot contain scripts. If the HTML contains a script, then the Enhanced Media Embed server will embed a link:mediaembed-server-config.html#_summary_cards[summary card].
...
The other option for developing a custom API endpoint is to return JSON in the link:mediaembed-server-integration.html#_tinymce_enhanced_media_embed_format[{productname} Enhanced Media Embed format].
...
* HTTP 200 (OK): link:mediaembed-server-integration.html#_ephoxembedobj[`+EphoxEmbedObj+`]
* HTTP 400 (User Error): link:mediaembed-server-integration.html#_errorobj[`+ErrorObj+`]
* HTTP 503 (Upstream Error): link:mediaembed-server-integration.html#_errorobj[`+ErrorObj+`]
* HTTP 500 (Unexpected Error): link:mediaembed-server-integration.html#_errorobj[`+ErrorObj+`]
...
* `+links+` (required)
** link:mediaembed-server-integration.html#_linksobj[LinksObj]
* `+rel+` (optional)
** link:mediaembed-server-integration.html#_relobj[RelObj]
* `+media+` (optional)
** link:mediaembed-server-integration.html#_mediaobj[MediaObj]
* `+html+` (optional)
...
* `+primary+` (required)
** Array of link:mediaembed-server-integration.html#_primaryrel[PrimaryRel]s
* `+technical+` (required)
** Array of link:mediaembed-server-integration.html#_technicalrel[TechnicalRel]s
* `+source+` (required)
** Array of link:mediaembed-server-integration.html#_sourcerel[SourceRel]s
...
* `+players+` (required)
** Array of link:mediaembed-server-integration.html#_linkobj[LinkObj]s
* `+thumbnails+` (required)
** Array of link:mediaembed-server-integration.html#_linkobj[LinkObj]s
* `+apps+` (required)
** Array of link:mediaembed-server-integration.html#_linkobj[LinkObj]s
...
** Integer
* `+width+` (required)
** link:mediaembed-server-integration.html#_dimensionboundobj[DimensionBoundObj]
* `+height+` (required)
** link:mediaembed-server-integration.html#_dimensionboundobj[DimensionBoundObj]
