...
See the docs on xref:mediaembed-server-config.adoc#configure_a_custom_endpoint[configuring a custom endpoint] for details on getting the Enhanced Media Embed server to utilize your custom API.

=== OEmbed endpoint

This is a popular choice and many CMSs have existing plugins that support oEmbed. While you can create custom HTML embeds this way, they cannot contain scripts. If the HTML contains a script, then the Enhanced Media Embed server will embed a xref:mediaembed-server-config.adoc#summary_cards[summary card].
...
The other option for developing a custom API endpoint is to return JSON in the xref:tinymce_enhanced_media_embed_format[{productname} Enhanced Media Embed format].
...
[[tinymce_enhanced_media_embed_format]]
