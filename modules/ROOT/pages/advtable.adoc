...
A numeric and alpha value series are available by default. The available value series can be configured using the xref:#_advtable_value_series[`+advtable_value_series+` option].
...
For events applicable to the Advanced Tables plugin, see: xref:table.adoc#_events[Table plugin - Events].
