...
An array of supported domains for the images (with CORS enabled on them) can be supplied to {productname} via the `+pass:a[{plugincode}]_cors_hosts+` option.
...
NOTE: `+pass:a[{plugincode}]_cors_hosts+` is *not* required when enabling this plugin via xref:editor-and-features.adoc[{cloudname}].

Type: `+String[]+`

Default Value: `+[]+`

=== Example: `+pass:a[{plugincode}]_cors_hosts+`
