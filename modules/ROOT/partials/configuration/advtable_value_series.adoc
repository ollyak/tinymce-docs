...
|generator |`+(info: GeneratorInfo, rowIndex: number, columnIndex: number) => GeneratorResult+` |Required |For details on creating a value series generator, see: xref:creating_a_value_series_generator[Creating a value series generator].
|===
