...
|`+documentId+` |`+string+` | The document ID from the xref:#_rtc_document_id[`+rtc_document_id+`] option.
|===
...
|===
|Field |Type |Description
|`+token+` |`+string+` | A generated JWT token. This token should be signed with a private key as described in xref:rtc-jwt-authentication.adoc#_jwt_endpoint_requirements[JWT authentication].
|===
