...
By default, a user's unique ID (`+userId+`, the `+sub+` field from their xref:rtc-jwt-authentication.adoc#_required_jwt_claims_for_real_time_collaboration[JWT]) will be displayed as the username in remote caret tooltips.
...
Only the `+fullName+` is required, but the whole object will be stored within the RTC caret information for use with the xref:#_rtc_client_connected[`+rtc_client_connected+`] and xref:#_rtc_client_disconnected[`+rtc_client_disconnected+`] callbacks to avoid duplicate lookup queries. The user details are also passed to the `+RtcClientConnected+` and `+RtcClientDisconnected+` events.
...
|any custom field | `+any+` | Extra user data for use in the xref:#_rtc_client_connected[`+rtc_client_connected+` API].
|===
