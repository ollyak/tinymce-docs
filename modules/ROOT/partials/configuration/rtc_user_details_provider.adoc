...
By default, a user's unique ID (`+userId+`, the `+sub+` field from their xref:rtc-jwt-authentication.adoc#required_jwt_claims_for_real_time_collaboration[JWT]) will be displayed as the username in remote caret tooltips.
...
|any custom field | `+any+` | Extra user data for use in the xref:rtc_client_connected[`+rtc_client_connected+` API].
|===
